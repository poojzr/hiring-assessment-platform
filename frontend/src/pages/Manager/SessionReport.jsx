...
import {
  ArrowLeft, User, Mail, Briefcase,
  AlertTriangle, Download, Play,
...
export default function SessionReport() {
  ...
  const fetchReport = async () => {
    setLoading(true)
    try {
      const response = await apiClient.get(`/manager/sessions/${sessionId}/report`)
  ...
  const exportReport = async (format) => {
    try {
      const response = await apiClient.get(`/manager/candidates/${report.candidate.id}/export`, {
        params: { format },
        responseType: 'blob'
      })
  ...
  const handleOverride = async () => {
    ...
    setSubmitting(true)
    try {
      await apiClient.post(`/manager/sessions/${sessionId}/override`, {
